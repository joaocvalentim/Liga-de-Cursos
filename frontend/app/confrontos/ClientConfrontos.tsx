// frontend/app/confrontos/ClientConfrontos.tsx
"use client";
...
export default function ClientConfrontos() {
  const searchParams = useSearchParams();
  const isAuthed = useAuthFlag();

  // filtros vindos da URL (opcionais)
  const cursoId = searchParams.get("curso");
  const fasesParam = searchParams.get("fases");   // "QF,SF,FINAL,THIRD"
  const estadoParam = searchParams.get("estado"); // "SCHEDULED,FT"
  ...
  const [myMatchVotes, setMyMatchVotes] = useState<Record<number, number>>({});

  // filtros (multi-seleção)
  const [stageFilter, setStageFilter] = useState<Set<Match["stage"]>>(new Set());
  const [statusSet, setStatusSet] = useState<Set<"SCHEDULED" | "FT">>(
    new Set()
  );
  const [courseFilter, setCourseFilter] = useState<Set<number>>(new Set());
  ...
  useEffect(() => {
    if (cursoId) setCourseFilter(new Set([Number(cursoId)]));
  ...
  const filtered = useMemo(() => {
    ...
    let list = matches.filter((m) => {
      if (hasStatuses && !statusSet.has(m.status as "SCHEDULED" | "FT")) return false;
